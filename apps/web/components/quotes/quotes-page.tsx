...
export function QuotesPage() {
      ...
      try {
        ...
        const details = await Promise.all(
          base.map(async (quote) => {
            try {
              return await api.getQuote(quote.id);
            } catch {
              return quote;
            }
          }),
        ...
        setError(
          cause instanceof ApiError && cause.status === 503
            ? "La API no está disponible. Comprueba que PostgreSQL y el servicio API estén iniciados."
            : "Se produjo un error inesperado al consultar los presupuestos.",
        );
      } finally {
        setLoading(false);
      }
  ...
  function handleRowClick(
    ...
    if (
      target.closest(
        'a, button, input, select, textarea, [role="menuitem"], [role="menu"], [data-radix-menu-content]',
      )
    ) {
      return;
    }
  ...
  async function duplicate(quote: QuoteRecord) {
    try {
      ...
      push("No se pudo duplicar el presupuesto", "error");
    }
  }
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Crea, revisa y continúa trabajos en curso sin perder de vista su
            estado.
          </p>
        </div>
            ...
            <div>
              ...
              <h3>
                {query || filter !== "all"
                  ...
                  : "Tu primer presupuesto empieza aquí"}
              </h3>

              <p>
                {query || filter !== "all"
                  ? "Prueba con otra búsqueda o cambia el filtro."
                  : "Selecciona un cliente, añade un título y entra directamente al editor."}
              </p>
            ...
            <tbody>
              {visible.map((quote) => (
                <tr
                  ...
                  className="quote-row"
                  tabIndex={0}
                  aria-label={`Abrir presupuesto ${quote.reference}: ${quote.title}`}
                  onClick={(event) => handleRowClick(event, quote)}
                ...
                >
                  ...
                  <td>
                    <OriginBadge origin={quote.origin} />
                  </td>

                  <td className="money">
                    <strong>
                      {formatMoney(quote.calculation?.saleWithTax)}
                    </strong>
                  </td>
                    ...
                    <DropdownMenu.Root>
                      <DropdownMenu.Trigger asChild>
                        <button
                          ...
                          aria-label={`Acciones para ${quote.reference}`}
      ...
      <CreateQuoteDialog
        ...
        onCreated={(quote, method) => {
          push("Presupuesto creado");
          router.push(`/presupuestos/${quote.id}?inicio=${method}`);
        }}
      />
...
function CreateQuoteDialog({
  ...
  onCreated: (quote: QuoteRecord, method: "ia" | "manual") => void;
}) {
  ...
  const refreshClients = useCallback(async () => {
    try {
      const freshClients = await api.searchClients("");
      setClients(freshClients);
    } catch {
      // Si la API falla, mantenemos la última lista válida.
    }
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (quickClient && clientName.trim()) {
        selectedClientId = (
          await api.createClient({
            name: clientName.trim(),
            ...(clientDetails.taxId.trim()
              ? { taxId: clientDetails.taxId.trim() }
              : {}),
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        description={
          step === 1
            ? "Elige el camino más cómodo. Ambos terminan en el mismo editor guiado."
            : "Deja identificado el presupuesto antes de preparar los conceptos."
        }
        footer={
          step === 1 ? (
            <>
            ...
            <>
              ...
              <RippleButton
                ...
                disabled={
                  busy ||
                  !title.trim() ||
                  (quickClient ? !clientName.trim() : !clientId)
                }
              >
                {busy
                  ? "Creando…"
                  : method === "ia"
                    ? "Crear y preparar con IA"
                    : "Crear y añadir conceptos"}
      ...
      >
        ...
        >
          <span data-active={step === 1}>1. Camino</span>
          <span data-active={step === 2}>2. Cliente y trabajo</span>
        </div>
        ...
        {step === 1 ? (
          <div className="start-paths">
            <button
              ...
              data-selected={method === "ia"}
              onClick={() => setMethod("ia")}
            >
              <span className="start-path-icon">
                <Bot />
              </span>

              <span>
                <strong>Preparar con IA</strong>
                <small>
                  Copia unas instrucciones, procesa las ofertas y revisa el
                  JSON antes de importar.
                </small>

                <em>
                  <Sparkles />
                  Recomendado para ofertas de proveedor
                </em>
              </span>
            </button>
            ...
            >
              ...
              <span>
                <strong>Crear paso a paso</strong>
                <small>
                  Añade material, mano de obra y desplazamientos desde
                  formularios breves.
                </small>

                <em>Control manual completo</em>
              </span>
            </button>
          </div>
        ) : (
          ...
          >
            <div className="field span-2">
              <label className="field-label" htmlFor="quote-title">
                Nombre del trabajo
              </label>
              ...
              <input
                ...
                placeholder="Ej. Climatización vivienda Las Palmas"
                required
              />
            </div>
            ...
            <div className="field span-2">
              <div className="field-label-row">
                <label className="field-label" htmlFor="quote-client">
                  Cliente
                </label>
                ...
                <button
                  ...
                  onClick={() => setQuickClient((value) => !value)}
                >
                  <UserPlus />
                  {quickClient
                    ? "Elegir existente"
              ...
              {quickClient ? (
                ...
                <SearchableSelect
                  ...
                  options={clients.map((client) => ({
                    ...
                    label: client.name,
                    description:
                      [client.taxId, client.email]
                        .filter(Boolean)
                        .join(" · ") || "Cliente guardado",
              ...
              <span className="field-hint">
                El cliente quedará asociado al crear el presupuesto.
              </span>
            </div>
            ...
            <div className="notice span-2">
              ...
              <span>
                ...
                {method === "ia"
                  ? "te guiaremos para extraer y revisar las ofertas."
                  : "entrarás directamente en los conceptos."}
              </span>
            </div>
